...
class NemotronClient:
    ...
    def _offline_simulated_reasoning(
        ...
        if "stop" in last_user_msg or "distance" in last_user_msg or "brake" in last_user_msg and "calc" in last_user_msg:
            ...
            text = (
                ...
                f"Battery status is {res['ev_battery_subsystem']['status']} at {res['ev_battery_subsystem']['state_of_charge_pct']}% charge and {res['ev_battery_subsystem']['max_cell_temp_c']}°C cell temp. "
                f"Tire pressures and traction are currently {res['tires_and_traction']['traction_status']}."
            )

        elif "procedure" in last_user_msg or "emergency" in last_user_msg or "fade" in last_user_msg:
